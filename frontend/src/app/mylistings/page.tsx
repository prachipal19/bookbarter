...
const MyListings: React.FC = () => {
  ...
  const fetchUserListings = async () => {
    try {
      ...
      const response = await fetch(`/api/listings/user-listings`, {
        headers: {
          Authorization: token,
        },
      });
  ...
  const handleDeleteListing = async (isbn: string) => {
    try {
      ...
      const response = await fetch(`/api/listings/delete?isbn=${isbn}`, {
        method: "DELETE",
        headers: {
          Authorization: token,
        },
      ...
      setListings((prevListings) =>
        prevListings.filter((listing) => listing.ISBN !== isbn)
      );
  ...
  return (
    <Layout>
      <div className="container">
        ...
        <Button variant="success" onClick={() => setShowAddBook(true)}>
          Add Book
        </Button>{" "}
        <br />
        <br />
        {showAddBook && <AddBook onClose={() => setShowAddBook(false)} />}{" "}
        {/* Close the AddBook form */}
        {listings.map((listing) => (
          ...
          >
            ...
            <div style={{ marginLeft: "auto" }}>
              ...
              <Button
                variant="danger"
                onClick={() => handleDeleteListing(listing.ISBN)}
              >
                Delete
              </Button>
            </div>
